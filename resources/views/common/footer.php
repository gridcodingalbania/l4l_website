<footer class="footer-element">
<div class="overflow-x-auto relative" >
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <tbody>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700 ">
                <th id="th" scope="row" class="pt-7  py-4 whitespace-nowrap dark:text-white">
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Store</span>
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Scopri e acquista</span>
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Servizi</span>
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Account</span>
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Roar nearby</span>
                </th>
            </tr>
            <tr class="container border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                    <div class="flex justify-between w-full">
                        <div>
                        <span class="cursor-pointer">Note Legali</span>
                        </div>
                        <div>
                            <button onclick="toggleText(1)">
                                <svg id="plus-icon1" width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 5C10 5.30683 9.75127 5.55556 9.44444 5.55556H5.63492V9.36508C5.63492 9.71574 5.35066 10 5 10C4.64934 10 4.36508 9.71574 4.36508 9.36508V5.55556H0.555555C0.248731 5.55556 0 5.30683 0 5C0 4.69318 0.248731 4.44444 0.555555 4.44444H4.36508V0.634921C4.36508 0.284264 4.64934 0 5 0C5.35066 0 5.63492 0.284264 5.63492 0.634921V4.44444H9.44445C9.75127 4.44444 10 4.69318 10 5Z" fill="#707070"/>
                                </svg>
                                <svg id="minus-icon1" class="hidden" width="10" height="2" viewBox="0 0 10 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 0.555556C10 0.86238 9.75127 1.11111 9.44444 1.11111H5.63492L4.36508 1.11113L0.555554 1.11111C0.24873 1.11111 0 0.862381 0 0.555557V0.555557C0 0.248731 0.248731 0 0.555557 0H4.36508H5.63492H9.44444C9.75127 0 10 0.248731 10 0.555556V0.555556Z" fill="#707070"/>
                                </svg>
                            </button>
                        </div>
                    </div>
                    <div id="toggle-text1"  class="hidden mt-3 whitespace-pre-wrap w-4/5 2xl:w-4/5 xl:w-4/5 lg:w-4/5 md:w-4/5">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sem morbi non nunc, pellentesque nec. Vitae orci, in adipiscing sed integer est. Sed duis habitant mauris sit risus neque ut ut. Eget risus lectus venenatis facilisis Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sem morbi non nunc, pellentesque nec.
                    </div>
                    
                </th>
            </tr>
            <tr class="container border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                    <div class="flex justify-between w-full">
                        <div>
                        <span class="cursor-pointer">Impressum</span>
                        </div>
                        <div>
                            <button class="align-right" onclick="toggleText(2)">
                                <svg id="plus-icon2" width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 5C10 5.30683 9.75127 5.55556 9.44444 5.55556H5.63492V9.36508C5.63492 9.71574 5.35066 10 5 10C4.64934 10 4.36508 9.71574 4.36508 9.36508V5.55556H0.555555C0.248731 5.55556 0 5.30683 0 5C0 4.69318 0.248731 4.44444 0.555555 4.44444H4.36508V0.634921C4.36508 0.284264 4.64934 0 5 0C5.35066 0 5.63492 0.284264 5.63492 0.634921V4.44444H9.44445C9.75127 4.44444 10 4.69318 10 5Z" fill="#707070"/>
                                </svg>
                                <svg id="minus-icon2" class="hidden" width="10" height="2" viewBox="0 0 10 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 0.555556C10 0.86238 9.75127 1.11111 9.44444 1.11111H5.63492L4.36508 1.11113L0.555554 1.11111C0.24873 1.11111 0 0.862381 0 0.555557V0.555557C0 0.248731 0.248731 0 0.555557 0H4.36508H5.63492H9.44444C9.75127 0 10 0.248731 10 0.555556V0.555556Z" fill="#707070"/>
                                </svg>
                            </button>
                        </div>
                    </div>
                    <div id="toggle-text2"  class="hidden mt-3 whitespace-pre-wrap w-4/5 2xl:w-4/5 xl:w-4/5 lg:w-4/5 md:w-4/5">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sem morbi non nunc, pellentesque nec. Vitae orci, in adipiscing sed integer est. Sed duis habitant mauris sit risus neque ut ut. Eget risus lectus venenatis facilisis Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sem morbi non nunc, pellentesque nec.
                    </div>
                    
                </th>
            </tr>
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                <th id="th" scope="row" class="py-4 whitespace-nowrap dark:text-white">
                <span class="cursor-pointer">Contattaci</span>
                </th>
            </tr>
            <tr class="border-none border-inherit" >
                <th id="th" scope="row" class="text-black pt-20 py-4 whitespace-nowrap dark:text-white">
                Italia
                </th>
            </tr>
            <tr class="border-none border-b-none border-inherit 2xl:flex xl:flex/ lg:flex grid 2xl:justify-between xl:justify-between lg:justify-between whitespace-normal">
                <div>
                    <th id="th" scope="row" class="pt-3 py-4 whitespace-nowrap dark:text-white">
                    Copyright © 2022 Roar Europe Ltd. Tutti i diritti riservati.<br>
                    <span class="text-black"> Norme sulla privacy  |  Utilizzo dei cookie <br>
                    Condizioni dʼuso |  Vendite e rimborsi </span>  <br>
                    Designed by Think Djungle <br>
                    </th>
                </div>
                <div>
                    <th id="th" scope="row" class="pt-3 py-4 whitespace-nowrap dark:text-white">
                    <div class="flex mx-auto mt-6 space-x-5  justify-center">
                        <a href="#" class="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        <svg width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4.1887 0C1.88524 0 0 1.84331 0 4.09562V16.8051C0 19.0573 1.8852 20.9 4.1887 20.9H17.187C19.4905 20.9 21.375 19.0573 21.375 16.8051V4.09562C21.375 1.84335 19.4905 0 17.187 0H4.1887ZM5.24222 3.44892C6.34668 3.44892 7.02698 4.15787 7.04798 5.08977C7.04798 6.00109 6.34664 6.72996 5.22086 6.72996H5.20014C4.1167 6.72996 3.41643 6.00113 3.41643 5.08977C3.41643 4.15789 4.13789 3.44892 5.2422 3.44892H5.24222ZM14.76 7.8045C16.8841 7.8045 18.4764 9.16197 18.4764 12.0791V17.5248H15.2483V12.4442C15.2483 11.1675 14.7811 10.2965 13.6129 10.2965C12.7211 10.2965 12.1895 10.8835 11.9562 11.4507C11.8709 11.6536 11.8499 11.937 11.8499 12.2208V17.5248H8.62191C8.62191 17.5248 8.66427 8.91838 8.62191 8.02723H11.8506V9.37219C12.2796 8.72505 13.0469 7.80448 14.76 7.80448V7.8045ZM3.60683 8.02792H6.83486V17.5249H3.60683V8.02792Z" fill="#101010"/>
                        </svg>
                        </a>
                        <a href="#" class="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        <svg width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M6.51675 0.0627C7.6311 0.0114 7.9864 0 10.825 0C13.6636 0 14.0189 0.01235 15.1323 0.0627C16.2457 0.11305 17.0057 0.2907 17.6707 0.54815C18.367 0.8113 18.9988 1.22265 19.5213 1.75465C20.0533 2.2762 20.4637 2.907 20.7259 3.6043C20.9843 4.2693 21.161 5.0293 21.2123 6.1408C21.2636 7.25705 21.275 7.61235 21.275 10.45C21.275 13.2886 21.2626 13.6439 21.2123 14.7582C21.1619 15.8697 20.9843 16.6297 20.7259 17.2947C20.4637 17.9921 20.0526 18.624 19.5213 19.1463C18.9988 19.6783 18.367 20.0887 17.6707 20.3509C17.0057 20.6093 16.2457 20.786 15.1342 20.8373C14.0189 20.8886 13.6636 20.9 10.825 20.9C7.9864 20.9 7.6311 20.8876 6.51675 20.8373C5.40525 20.7869 4.64525 20.6093 3.98025 20.3509C3.28287 20.0887 2.65103 19.6776 2.1287 19.1463C1.59706 18.6244 1.18565 17.9929 0.92315 17.2957C0.6657 16.6307 0.489 15.8707 0.4377 14.7592C0.3864 13.6429 0.375 13.2876 0.375 10.45C0.375 7.6114 0.38735 7.2561 0.4377 6.1427C0.48805 5.0293 0.6657 4.2693 0.92315 3.6043C1.18604 2.90708 1.59776 2.27555 2.12965 1.7537C2.65124 1.22218 3.28245 0.810771 3.9793 0.54815C4.6443 0.2907 5.4043 0.114 6.5158 0.0627H6.51675ZM15.0477 1.9437C13.9457 1.89335 13.6151 1.8829 10.825 1.8829C8.03485 1.8829 7.70425 1.89335 6.60225 1.9437C5.5829 1.99025 5.03 2.1603 4.6614 2.30375C4.17405 2.49375 3.8254 2.7189 3.45965 3.08465C3.11294 3.42195 2.84612 3.83257 2.67875 4.2864C2.5353 4.655 2.36525 5.2079 2.3187 6.22725C2.26835 7.32925 2.2579 7.65985 2.2579 10.45C2.2579 13.2401 2.26835 13.5707 2.3187 14.6727C2.36525 15.6921 2.5353 16.245 2.67875 16.6136C2.84595 17.0668 3.1129 17.4781 3.45965 17.8154C3.7969 18.1621 4.20825 18.4291 4.6614 18.5963C5.03 18.7397 5.5829 18.9098 6.60225 18.9563C7.70425 19.0067 8.0339 19.0171 10.825 19.0171C13.6161 19.0171 13.9457 19.0067 15.0477 18.9563C16.0671 18.9098 16.62 18.7397 16.9886 18.5963C17.476 18.4063 17.8246 18.1811 18.1904 17.8154C18.5371 17.4781 18.8041 17.0668 18.9713 16.6136C19.1147 16.245 19.2848 15.6921 19.3313 14.6727C19.3817 13.5707 19.3921 13.2401 19.3921 10.45C19.3921 7.65985 19.3817 7.32925 19.3313 6.22725C19.2848 5.2079 19.1147 4.655 18.9713 4.2864C18.7813 3.79905 18.5561 3.4504 18.1904 3.08465C17.853 2.73797 17.4424 2.47115 16.9886 2.30375C16.62 2.1603 16.0671 1.99025 15.0477 1.9437V1.9437ZM9.49025 13.6714C10.2357 13.9817 11.0657 14.0236 11.8386 13.7899C12.6115 13.5562 13.2792 13.0615 13.7278 12.3901C14.1764 11.7188 14.3781 10.9125 14.2982 10.109C14.2184 9.30557 13.8622 8.55472 13.2903 7.98475C12.9257 7.6204 12.4849 7.34142 11.9995 7.16789C11.5142 6.99436 10.9964 6.93059 10.4835 6.98118C9.97053 7.03178 9.47519 7.19547 9.03311 7.46047C8.59103 7.72548 8.21321 8.08521 7.92684 8.51376C7.64047 8.94231 7.45268 9.42902 7.37699 9.93886C7.3013 10.4487 7.33959 10.969 7.48911 11.4622C7.63863 11.9555 7.89566 12.4095 8.24168 12.7915C8.58771 13.1735 9.01413 13.474 9.49025 13.6714ZM7.0269 6.6519C7.52567 6.15313 8.1178 5.75748 8.76948 5.48754C9.42116 5.21761 10.1196 5.07868 10.825 5.07868C11.5304 5.07868 12.2288 5.21761 12.8805 5.48754C13.5322 5.75748 14.1243 6.15313 14.6231 6.6519C15.1219 7.15067 15.5175 7.7428 15.7875 8.39448C16.0574 9.04616 16.1963 9.74463 16.1963 10.45C16.1963 11.1554 16.0574 11.8538 15.7875 12.5055C15.5175 13.1572 15.1219 13.7493 14.6231 14.2481C13.6158 15.2554 12.2496 15.8213 10.825 15.8213C9.40044 15.8213 8.03422 15.2554 7.0269 14.2481C6.01958 13.2408 5.45368 11.8746 5.45368 10.45C5.45368 9.02544 6.01958 7.65922 7.0269 6.6519V6.6519ZM17.3876 5.8786C17.5112 5.76201 17.6101 5.6218 17.6786 5.46628C17.747 5.31075 17.7836 5.14308 17.786 4.97318C17.7885 4.80329 17.7569 4.63462 17.693 4.47717C17.6291 4.31972 17.5343 4.17669 17.4142 4.05654C17.294 3.93639 17.151 3.84157 16.9935 3.77769C16.8361 3.71381 16.6674 3.68218 16.4975 3.68465C16.3276 3.68713 16.1599 3.72367 16.0044 3.79211C15.8489 3.86055 15.7087 3.9595 15.5921 4.0831C15.3653 4.32348 15.2412 4.64277 15.246 4.97318C15.2508 5.3036 15.3842 5.61913 15.6179 5.8528C15.8516 6.08647 16.1671 6.21987 16.4975 6.22468C16.8279 6.2295 17.1472 6.10536 17.3876 5.8786V5.8786Z" fill="#101010"/>
                        </svg>
                        </a>
                        <a href="#" class="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        <svg width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10.7749 21C4.97575 21 0.274902 16.2991 0.274902 10.5C0.274902 4.70085 4.97575 0 10.7749 0C16.5741 0 21.2749 4.70085 21.2749 10.5C21.2686 16.296 16.5709 20.9937 10.7749 21ZM6.28825 13.4263C6.1528 13.4263 6.0184 13.4431 5.8861 13.4746L5.7286 13.5177L5.1049 14.5855C4.99098 14.7756 4.95439 15.0021 5.00271 15.2183C5.05103 15.4346 5.18056 15.624 5.36454 15.7474C5.54852 15.8709 5.77289 15.919 5.9913 15.8817C6.20971 15.8445 6.40548 15.7248 6.53815 15.5473L6.60325 15.4497L7.43275 14.0101C7.29831 13.8303 7.12409 13.6841 6.92372 13.5828C6.72335 13.4815 6.50226 13.428 6.27775 13.4263H6.28825ZM12.0465 6.993C11.7198 7.29989 11.5032 7.70577 11.4301 8.148C11.313 8.75777 11.4252 9.38929 11.7451 9.92145L14.9371 15.4507C15.0482 15.6377 15.2253 15.7761 15.4336 15.8387C15.6418 15.9013 15.8659 15.8835 16.0617 15.7888C16.2568 15.6926 16.4085 15.5262 16.4864 15.323C16.5642 15.1198 16.5625 14.8947 16.4817 14.6926L16.4302 14.5876L15.5902 13.1271H16.8397C17.0585 13.1253 17.2685 13.0405 17.4272 12.8899C17.586 12.7393 17.6817 12.534 17.695 12.3156C17.7083 12.0972 17.6383 11.8818 17.499 11.713C17.3598 11.5442 17.1617 11.4346 16.9447 11.4061L16.8397 11.3998H14.5927L12.289 7.40985L12.0486 6.98985L12.0465 6.993ZM4.7479 11.3925C4.63507 11.3928 4.5234 11.4153 4.41929 11.4588C4.31518 11.5023 4.22067 11.5659 4.14118 11.646C4.06169 11.7261 3.99878 11.821 3.95604 11.9255C3.91331 12.0299 3.8916 12.1417 3.89215 12.2545C3.89215 12.6892 4.21555 13.0567 4.6471 13.1124L4.7521 13.1176H12.6061C12.693 12.9431 12.7368 12.7503 12.7337 12.5553C12.7305 12.3603 12.6807 12.169 12.5883 11.9973C12.5096 11.8365 12.3925 11.6976 12.2472 11.593C12.1019 11.4884 11.9331 11.4213 11.7556 11.3977L11.6286 11.3914H8.95L12.6544 4.977C12.7685 4.77816 12.7994 4.5423 12.7405 4.32075C12.6986 4.16528 12.6141 4.02459 12.4965 3.91453C12.379 3.80446 12.2331 3.72938 12.0752 3.69774C11.9173 3.6661 11.7537 3.67914 11.6028 3.73541C11.4519 3.79168 11.3198 3.88894 11.2212 4.01625L11.1561 4.11285L10.7749 4.77855L10.398 4.11285C10.2868 3.92568 10.1095 3.78705 9.90104 3.72444C9.69258 3.66182 9.4682 3.67978 9.27235 3.77475C9.07727 3.8714 8.92573 4.03796 8.84791 4.24129C8.77008 4.44462 8.77167 4.66979 8.85235 4.872L8.9038 4.977L9.7753 6.51L6.95605 11.3925H4.7479Z" fill="#101010"/>
                        </svg>
                        </a>
                        <a href="#" class="text-gray-400 hover:text-gray-900 dark:hover:text-white">
                        <svg width="26" height="21" viewBox="0 0 26 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M25.263 4.85363C25.1535 7.21846 23.4535 10.4536 20.1678 14.5638C16.7713 18.8548 13.8939 21.0003 11.5392 21.0003C10.0809 21.0003 8.84871 19.6913 7.84157 17.0815C7.17014 14.684 6.4999 12.2865 5.82133 9.89479C5.0749 7.28496 4.27371 5.97596 3.41062 5.97596C3.22609 5.97596 2.56895 6.36096 1.45228 7.11929L0.274902 5.64346C1.50705 4.58646 2.72728 3.53763 3.9249 2.48179C5.56776 1.09929 6.80824 0.37596 7.62966 0.30246C9.57371 0.12396 10.7725 1.41196 11.2249 4.18163C11.7106 7.16363 12.0463 9.02329 12.238 9.74663C12.7999 12.2246 13.4166 13.4601 14.0868 13.4601C14.607 13.4601 15.3951 12.6575 16.4428 11.0498C17.4904 9.44213 18.0511 8.22063 18.1273 7.38296C18.2773 5.99463 17.7166 5.30396 16.4428 5.30396C15.8475 5.30396 15.2309 5.43696 14.5939 5.70296C15.8201 1.79113 18.1678 -0.109373 21.6332 0.0037934C24.2011 0.0702934 25.413 1.69079 25.263 4.85246V4.85363Z" fill="#101010"/>
                        </svg>
                        </a>
                    </div>
                    </th>
                </div>
            </tr>
        </tbody>   
    </table>
</div>
</footer>
<script>
function toggleText(num) {
  var text = document.getElementById("toggle-text" + num);
  var plus = document.getElementById("plus-icon" + num);
  var minus = document.getElementById("minus-icon" + num);
  if (text.style.display === "none") {
    text.style.display = "block";
    plus.style.display = "none";
    minus.style.display = "block";
  } else {
    text.style.display = "none";
    plus.style.display = "block";
    minus.style.display = "none";
  }
}
</script>